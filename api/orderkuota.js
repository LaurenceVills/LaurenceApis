...
class OrderKuota {
  ...
  async generateQr(amount = "") {
    ...
    if (response && response.success && response.qris_merchant_terms && response.qris_merchant_terms.results != null) {
...
function generateTransactionId() {
  return `LAURENCE-${crypto.randomBytes(2).toString("hex").toUpperCase()}`;
}
